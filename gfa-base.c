...
gfa_t *gfa_init(void)
{
	...
	//inizializzazione delle due hashmap, h_names è quella principale.
...
int32_t gfa_add_seg(gfa_t *g, const char *name)
{
	...
	//hasmap che mappa i nomi dei segmenti agli ID
	seghash_t *h = (seghash_t*)g->h_names; 

	//cerca di inserire "name" nella hasmap, se c'è già allora restituisce l'indice k e setta absent a 0. Altrimenti trova una posizione libera, restituisce l'indice k e setta absent == 1.
	k = kh_put(seg, h, name, &absent);
	if (absent) {
		gfa_seg_t *s;

		//se il numero di segmenti "n_seg" raggiunge la capacità massima "m_seg" allora la capacità viene raddoppiata
		if (g->n_seg == g->m_seg) {
			uint32_t old_m = g->m_seg;
			//qui avviene il raddoppio
		...
		//ottiene il puntatore alla nuova posizione e incrementa n_seg giustamente. L'id numerico del segmento sarà semplicmenete n_seg - 1.
		s = &g->seg[g->n_seg++];

		//alloca memoria e copia il nome del segmento. Associa poi la stringa appena copiata alla chiave dell'hashtable
		kh_key(h, k) = s->name = gfa_strdup(name);
		//setta i campi non inizializzati
		s->del = s->len = 0;
		s->snid = s->soff = s->rank = -1;

		//associa l'id numerico all'indice k dell'hashtable
...
gfa_arc_t *gfa_add_arc1(gfa_t *g, uint32_t v, uint32_t w, int32_t ov, int32_t ow, int64_t link_id, int comp)
{
	...
	//controlla gli archi presenti (g->n_arc) e vede che non siano arrivati al max (g->m_arc)
	if (g->m_arc == g->n_arc) {
		uint64_t old_m = g->m_arc;
		//se si, raddoppia la memoria allocata per gli archi
	...
	//puntatore al nuovo arco e g->n_arc viene incrementato. L'ID dell'arco sarà giustamente n_arc - 1 proprio come l'ID dell'hashmap quando salvi i nomi dei nodi.
	a = &g->arc[g->n_arc++];
	//l'ID orientato viene salvato in a->v_lv e spostato in alto di 32 bit 
	a->v_lv = (uint64_t)v << 32;
	//assegnati  i valori di overlap e il valore di arrivo w.
	a->w = w, a->ov = ov, a->ow = ow, a->rank = -1;

	//queste sono cose relative in generale a parametri dell'arco o sono cose che hanno a che fare con link_id e comp che dalla chiamata di gfa_parse_L sono settati a -1 e 0 quindi qui non serve.
